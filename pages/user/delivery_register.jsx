import Link from "next/link";
import { Container, Row, Col } from "react-bootstrap";
import { FaFacebookF, FaGooglePlusG } from "react-icons/fa";
import BreadCrumb from "../../components/breadcrumb/BreadCrumb";

const navigationDefault = ["Home", "User", "Delivery Boy Register"]
const DeliveryBoyRegister = () => {
    return (
        <div>
            <BreadCrumb title="Delivery Boy Register" navigation={navigationDefault} />
            <section className="login section-padding">
                <div className="card delivery__card">
                    <form>
                        <h2 className="title mb-5">Delivery Boy Registeration </h2>
                        {/* <p className="subtitle">already have an account? <a href="login.html"> Login</a></p> */}



                        <div className="email-login">
                            <div className="row justify-content-between">
                                <div className="col-md-6">
                                    <label htmlFor="firstName"> <b>First Name*</b></label>
                                    <input type="text" placeholder="Enter First Name" name="firstName" required />
                                    <label htmlFor="lastName"> <b>Last Name*</b></label>
                                    <input type="text" placeholder="Enter Last Name" name="lastName" required />
                                    <label htmlFor="contactNumber"> <b>Contact Number*</b></label>
                                    <input type="text" placeholder="Enter Contact Number" name="contactNumber" required />
                                    <label htmlFor="gender"> <b>Gender</b></label>
                                    <select name="gender" id="gender">
                                        <option value="male">Male</option>
                                        <option value="female">Female</option>
                                        <option value="other">Other</option>
                                    </select>
                                    <label htmlFor="email"> <b>Email*</b></label>
                                    <input type="text" placeholder="Enter Email" name="email" required />
                                    <label htmlFor="pinCode"> <b>Pin Code</b></label>
                                    <input type="text" placeholder="Enter Pincode" name="pinCode" required />
                                    <label htmlFor="address"> <b>Address</b></label>
                                    <textarea name="address" id="address" cols="30" rows="5"></textarea>
                                    <label htmlFor="id_proof"> <b>ID Proof</b></label>
                                    <select name="id_proof" id="id_proof">
                                        <option value="bike">Bike</option>
                                        <option value="car">Car</option>
                                    </select>
                                    <label htmlFor="adhar_id_proof"> <b>ID Proof</b></label>
                                    <select name="adhar_id_proof" id="adhar_id_proof">
                                        <option value="Adhar">Adhar</option>
                                        <option value="License">License</option>
                                    </select>
                                    <label htmlFor="documents"> <b>Documents*</b></label>
                                    <input type="file" placeholder="Enter Contact Number" name="documents" required />
                                </div>
                                <div className="col-md-6">
                                    <label htmlFor="licenseNumber"> <b>License Number*</b></label>
                                    <input type="text" placeholder="Enter License Number" name="licenseNumber" required />
                                    <label htmlFor="license_img"> <b>License Image*</b></label>
                                    <input type="file" placeholder="Enter License Image" name="license_img" required />
                                    <label htmlFor="rc_book"> <b>RC Book Image*</b></label>
                                    <input type="file" placeholder="Enter RC Image" name="rc_book" required />
                                    <label htmlFor="userName"> <b>User Name</b></label>
                                    <input type="text" placeholder="Enter User Name" name="userName" required />
                                    <label htmlFor="psw"> <b>Password</b></label>
                                    <input type="password" placeholder="Enter Password" name="psw" required />
                                    <label htmlFor="cpsw"><b>Confirm Password</b></label>
                                    <input type="password" placeholder="Confirm Password" name="cpsw" required />
                                </div>
                            </div>


                        </div>
                        <button className="btn w-100 mt-3" type="submit">Register</button>
                        {/* <p className="or"><span>or</span></p>

                        <div className="social-login">
                            <button className="google-btn">
                                <img alt="Google"
                                    src="data:image/svg+xml;base64," />
                                <p className="btn-text">Sign in with Google</p>
                            </button>

                        </div> */}

                        {/* <a className="forget-pass mt-3" href="#">Forgot password?</a> */}
                    </form>
                </div>
            </section >
        </div>
    );
};

export default DeliveryBoyRegister;
